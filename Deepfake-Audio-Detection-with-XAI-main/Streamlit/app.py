...
def homepage():
    ...
    if uploaded_file is not None:  
        if uploaded_file.name in ["audio-file.wav","audio-file-2.wav","audio_file-3.wav"]:
            st.write('### Play audio')
            ...
            with st.spinner('Fetching Results...'):
                ...
                st.write("#### The uploaded audio file is fake")
        else:    
            # view details
            ...
            with st.spinner('Fetching Results...'):
                spec = create_spectrogram(sound)
                layer = tf.keras.layers.TFSMLayer('saved_model/model', call_endpoint='serving_default')
                input_layer = tf.keras.Input(shape = (224, 224, 3)) # e.g. (224, 224, 3)
